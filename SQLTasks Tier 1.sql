...
FROM (SELECT sub.facility, SUM(sub.cost) AS total_revenue
     ...
     GROUP BY sub.facility) AS sub_2
WHERE sub_2.total_revenue < 1000;
...
SELECT f.name AS facility, COUNT(f.name)
FROM Facilities AS f
...
SELECT f.name AS facility, COUNT(f.name),
	...
	SUM(case when month(starttime) = 9 then 1 else 0 end) as Sept,
